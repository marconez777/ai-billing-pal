-- ==========================================================
-- PROMPT 4: RLS Extra + Analytics Views
-- ==========================================================
...
-- ==========================================================
-- VIEW: Transfers (origem/destino consolidadas por grupo)
...
select
  ...
  min(tg.txn_date) as txn_date,
  -- origem é a linha com amount < 0; destino a com amount > 0
  max(case when tg.amount < 0 then tg.account_id end) as from_account_id,
  max(case when tg.amount < 0 then a_from.name end)    as from_account_name,
  max(case when tg.amount > 0 then tg.account_id end) as to_account_id,
  max(case when tg.amount > 0 then a_to.name end)     as to_account_name,
  -- valor absoluto (assumindo simetria)
  max(abs(tg.amount)) as amount_abs,
  -- descrição preferindo a da origem
  coalesce(
    max(case when tg.amount < 0 then tg.description end),
    max(case when tg.amount > 0 then tg.description end)
  ) as description
from public.transactions tg
left join public.accounts a_from on a_from.id = tg.account_id
left join public.accounts a_to   on a_to.id   = tg.account_id
where tg.transfer_group_id is not null
group by tg.user_id, tg.transfer_group_id;
