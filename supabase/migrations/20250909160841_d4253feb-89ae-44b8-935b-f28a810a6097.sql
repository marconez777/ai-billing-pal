...
with transfer_data as (
  select
    ...
    t.amount,
    t.description,
    row_number() over (partition by t.transfer_group_id order by t.amount) as rn
  from public.transactions t
  join public.accounts a on a.id = t.account_id
  where t.transfer_group_id is not null
),
from_accounts as (
  select * from transfer_data where amount < 0
),
to_accounts as (
  select * from transfer_data where amount > 0
)
select
  ...
  coalesce(f.transfer_group_id, t.transfer_group_id) as transfer_group_id,
  coalesce(f.txn_date, t.txn_date) as txn_date,
  f.account_id as from_account_id,
  f.account_name as from_account_name,
  t.account_id as to_account_id,
  t.account_name as to_account_name,
  abs(coalesce(f.amount, t.amount)) as amount_abs,
  coalesce(f.description, t.description) as description
from from_accounts f
full outer join to_accounts t on t.transfer_group_id = f.transfer_group_id;
